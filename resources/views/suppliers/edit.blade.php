<x-app-layout>
    <div id="pageLoader" class="fixed inset-0 bg-white dark:bg-gray-900 z-50 flex items-center justify-center">
        <div class="text-center">
            ...
            <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-2">Loading Edit Supplier Page...</h3>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden">
                <div class="px-6 py-8 sm:p-10">
                    <form action="{{ route('suppliers.update', $supplier) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        @method('PUT')
                                ...
                                <div id="photo-preview" class="relative">
                                    <div class="w-32 h-32 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 dark:bg-gradient-to-br dark:from-gray-700 dark:to-gray-600 flex items-center justify-center overflow-hidden border-4 border-white dark:border-gray-700 shadow-lg">
                                        @if($supplier->photo_profile)
                                            <img id="preview-image" src="{{ asset('storage/' . $supplier->photo_profile) }}" class="w-full h-full object-cover" alt="Profile preview">
                                        @else
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="store_name" id="store_name" value="{{ old('store_name', $supplier->store_name) }}"
                                           class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-3 transition duration-200"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="province_id" id="province_id" 
                                        ...
                                        @foreach(\Laravolt\Indonesia\Models\Province::all() as $province)
                                            <option value="{{ $province->id }}" {{ old('province_id', optional($supplier->address)->province_id) == $province->id ? 'selected' : '' }}>
                                                {{ $province->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="detail_address" id="detail_address" rows="3"
                                              class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-3 transition duration-200"
                                              placeholder="Street name, building number, additional info...">{{ old('detail_address', optional($supplier->address)->detail_address) }}</textarea>
                            ...
                            <div class="mt-6">
                                ...
                                <input type="hidden" name="latitude" id="latitude" value="{{ old('latitude', optional($supplier->address)->latitude) }}">
                                <input type="hidden" name="longitude" id="longitude" value="{{ old('longitude', optional($supplier->address)->longitude) }}">
                                ...
                                <div class="mt-3 grid grid-cols-2 gap-4">
                                    <div class="bg-blue-50 dark:bg-gray-700 rounded-lg p-3">
                                        ...
                                        <div id="lat-display" class="text-sm font-medium text-gray-900 dark:text-white">{{ optional($supplier->address)->latitude ?? 'Not set' }}</div>
                                    </div>
                                    <div class="bg-blue-50 dark:bg-gray-700 rounded-lg p-3">
                                        <span class="text-xs text-gray-500 dark:text-gray-400">Longitude:</span>
                                        <div id="lng-display" class="text-sm font-medium text-gray-900 dark:text-white">{{ optional($supplier->address)->longitude ?? 'Not set' }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
document.getElementById('photo_profile').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            const svgIcon = photoPreview.querySelector('svg');
...
// Initialize Map
let map, marker;
const defaultLat = {{ optional($supplier->address)->latitude ?? -6.2088 }};
const defaultLng = {{ optional($supplier->address)->longitude ?? 106.8456 }};

document.addEventListener('DOMContentLoaded', function() {
    map = L.map('map').setView([defaultLat, defaultLng], {{ optional($supplier->address)->latitude ? 15 : 13 }});
    ...
    // Load existing data - PERBAIKAN DI SINI
    const provinceId = '{{ old('province_id', optional($supplier->address)->province_id ?? '') }}';
    const cityId = '{{ old('city_id', optional($supplier->address)->city_id ?? '') }}';
    const districtId = '{{ old('district_id', optional($supplier->address)->district_id ?? '') }}';
    const villageId = '{{ old('village_id', optional($supplier->address)->village_id ?? '') }}';
        ...
        fetch(`/api/districts/${cityId}`)
            ...
            .then(data => {
                ...
                if (selectedDistrictId) {
                    loadVillages(selectedDistrictId, villageId);
                }
            })
            .catch(error => console.error('Error:', error));
    }
