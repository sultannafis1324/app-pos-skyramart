<x-app-layout>
    ...
    <div id="pageLoader" class="fixed inset-0 bg-white dark:bg-gray-900 z-50 flex items-center justify-center">
        <div class="text-center">
            ...
            <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-2">Loading Add New Customers Page...</h3>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center">
                    ...
                    <span class="ml-3 text-sm font-medium text-gray-900 dark:text-white">Customer Information</span>
                </div>
            </div>

            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden">
                <div class="px-6 py-8 sm:p-10">
                    <form action="{{ route('customers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="detail_address" id="detail_address" rows="3"
                                              class="block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-4 py-3 transition duration-200"
                                              placeholder="Street name, building number, RT/RW, etc...">{{ old('detail_address') }}</textarea>
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-end sm:space-x-3 space-y-3 sm:space-y-0 pt-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <button type="submit" 
                                    class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3 border border-transparent rounded-xl text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transform hover:scale-105">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // PERBAIKAN: Disable semua dropdown di bawah provinsi jika provinsi belum dipilih
    ...
    // PERBAIKAN: Cascading Dropdown - Province
        ...
        if (provinceId) {
            ...
            fetch(`/api/cities/${provinceId}`)
                ...
                .then(data => {
                    ...
                    citySelect.disabled = false; // Enable setelah data loaded
                })
                .catch(error => {
            ...
            // AUTO GEOCODE: Coba geocode jika detail address sudah terisi
            tryAutoGeocode();
        }
    });

    // PERBAIKAN: Cascading Dropdown - City
        ...
        if (cityId) {
            ...
            fetch(`/api/districts/${cityId}`)
                ...
                .then(data => {
                    ...
                    districtSelect.disabled = false; // Enable setelah data loaded
                })
                .catch(error => {
    ...
    // PERBAIKAN: Cascading Dropdown - District
        ...
        if (districtId) {
            ...
            fetch(`/api/villages/${districtId}`)
                ...
                .then(data => {
                    ...
                    villageSelect.disabled = false; // Enable setelah data loaded
                })
                .catch(error => {
    ...
    // AUTO GEOCODE saat village dipilih
    ...
    // PERBAIKAN: Auto Geocoding untuk detail address
    ...
    detailAddressInput.addEventListener('input', function() {
        ...
        // Geocode hanya jika alamat cukup panjang DAN minimal provinsi sudah dipilih
        if (address.length > 10) {
            geocodeTimeout = setTimeout(() => {
                tryAutoGeocode();
            }, 1500); // Delay 1.5 detik setelah user berhenti mengetik
        }
    });

    // FUNGSI BARU: Coba auto geocode
    function tryAutoGeocode() {
        const detailAddress = detailAddressInput.value.trim();
        
        // Harus ada detail address DAN minimal provinsi
...
// PERBAIKAN: Fungsi geocode yang lebih baik dengan fallback
function geocodeAddress(address) {
    ...
    // Siapkan beberapa versi alamat untuk dicoba (dari spesifik ke umum)
    ...
    // Versi 2: Tanpa detail address (hanya kelurahan/kecamatan/kota)
    let generalAddress = '';
    if (villageSelect.value && villageSelect.selectedIndex > 0) {
    ...
    fetch(url, {
        ...
        .then(data => {
            if (data && data.length > 0) {
                ...
                // Set marker dan pindahkan map dengan animasi
                setMarker(lat, lng);
                ...
                if (index === 0) {
                    ...
                    showNotification('📍 Lokasi area ditemukan. Geser marker untuk lokasi tepat.', 'info');
                }
            } else {
                // Coba alamat berikutnya
                console.log(`Attempt ${index + 1} failed, trying next variation...`);
                setTimeout(() => {
                    tryGeocodeWithFallback(addressVariations, index + 1);
                }, 500); // Delay 500ms antar request
            }
        })
        .catch(error => {
...
// PERBAIKAN: Notifikasi dengan tipe
